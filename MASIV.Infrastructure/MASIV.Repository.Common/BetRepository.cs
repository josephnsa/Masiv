...
namespace MASIV.Repository.Common
{
    public class BetRepository : IBetRepository
    {
        ...
        public BetRepository(ICommonRepository repo, IRouletteRepository rouletteRepo)
        {
            repository = repo;
        ...
        public bool CreateBet(Bet bet)
        {
            ...
            {
                throw new Exception(Utils.CreateMessageError(message: "La apuesta se intenta realizar en una ruleta que no existe o no esta abierta."));
            }

            if (!ValidateColor(color: bet.Color))
            {
                throw new Exception(Utils.CreateMessageError(message: "Los colores válidos son [Rojo] y [Negro]"));
            }
        ...
        {
            List<Bet> betsRoulette = GetAllByRouletteId(rouletteId: rouletteID);
