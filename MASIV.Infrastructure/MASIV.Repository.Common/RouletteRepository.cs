...
namespace MASIV.Repository.Common
{
    public class RouletteRepository : IRouletteRepository
    {
        ...
        private readonly string RouletteKey = Environment.GetEnvironmentVariable(variable: "RouletteKey");
        public RouletteRepository(ICommonRepository repo)
        {
            repository = repo;
        }
        public Roulette GetById(Guid rouletteId)
        {
            ...
            {
                throw new Exception(Utils.CreateMessageError($"No se encontró una ruleta con este id:{rouletteId}"));
            }
        ...
        public bool CloseRoulette(Guid rouletteId)
        {
            ...
            {
                throw new Exception(Utils.CreateMessageError($"La rouleta {rouletteId} ya se encuentra cerrada."));
            }
        ...
        private void ValidateOpenRoulette(Guid rouleteId)
        {
            ...
            if (rouletes.Any())
            {
                ...
                {
                    throw new Exception(Utils.CreateMessageError("Esta ruleta ya se encuentra abierta."));
                }

                if (rouletteOpenedExists != null)
                {
                    throw new Exception(Utils.CreateMessageError("Ya existe una ruleta abierta, cierrela antes de abrir una nueva."));
                }
            }
        }
    }
